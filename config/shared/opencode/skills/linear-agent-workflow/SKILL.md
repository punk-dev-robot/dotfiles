...
- Transport: `linear` CLI (always available), or Linear MCP tools when configured.
...
- `linear` CLI: `linear team list`, `linear issue list/create/update`, `linear project list` — confirm team/state/label IDs before sensitive writes.
- Linear MCP (when configured): `linear_list_issues` with pagination for broad reads, `linear_get_issue` before updating, `linear_save_issue` for create/update (patch ops preferred over full description overwrite).
