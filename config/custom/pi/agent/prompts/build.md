...
Invoke the /skill:incremental-implementation skill alongside /skill:test-driven-development.
...
3. **Plan if needed.** If there is no `tasks/plan.md`, invoke /skill:planning-and-task-breakdown to generate one.
...
6. **Stop and ask the user** (do not push through) when:
   - a test can't be made to pass or the build breaks without an obvious fix → follow /skill:debugging-and-error-recovery
   - the spec is ambiguous, or a task needs a decision the spec doesn't cover
   - a task is high-risk or irreversible — auth/permission changes, destructive data migrations, payments, deletions, deploys, anything touching secrets, **or anything you can't undo with `git revert`** → follow /skill:doubt-driven-development and get explicit sign-off before continuing
