---
description: Five-axis code review (correctness, readability, architecture, security, performance) via the reviewer subagent
argument-hint: "[scope]"
---

Review the current changes across all five axes. Scope: ${@:-the staged changes, or the most recent commit if nothing is staged}.

Do not review the diff yourself — delegate to the `reviewer` subagent. It runs a different
...
2. Launch one `reviewer` child with a self-contained brief: the scope, the intent of the
